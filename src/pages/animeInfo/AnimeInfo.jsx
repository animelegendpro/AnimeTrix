              ...
              isProducer ? (
                <Link
                  to={`/producer/${item
                    ...
                    .replace(/-+/g, "-")}`}
                  key={index}
                  className="cursor-pointer hover:text-[#FF6300]"
                >
                  {item}
        ...
        <div className="flex items-start z-10 px-14 py-[70px] bg-[#252434] bg-opacity-70 gap-x-8 max-[1024px]:px-6 max-[1024px]:py-10 max-[1024px]:gap-x-4 max-[575px]:flex-col max-[575px]:items-center max-[575px]:justify-center">
          ...
          <div className="flex flex-col ml-4 gap-y-5 max-[575px]:items-center max-[575px]:justify-center max-[575px]:mt-6 max-[1200px]:ml-0">
            <ul className="flex gap-x-2 items-center w-fit max-[1200px]:hidden">
              {[
                ...
                <li key={index} className="flex gap-x-3 items-center">
                  <Link
                    to={`/${link}`}
                    className="text-white hover:text-[#FF6300] text-[15px] font-semibold"
                  >
                    {text}
                  </Link>
            ...
            <Link
              ...
              className="flex gap-x-2 px-6 py-2 bg-[#FF6300] w-fit text-black items-center rounded-3xl mt-5"
            >
            ...
            <div className="flex gap-x-4 items-center mt-4 max-[575px]:w-full max-[575px]:justify-center max-[320px]:hidden">
              ...
              <div className="flex flex-col w-fit">
                <p className="text-[15px] font-bold text-[#FF6300]">
                  Share Anime
                </p>
                ...
                <div className="flex flex-wrap gap-2">
                  {info.Genres.map((genre, index) => (
                    <Link
                      ...
                      key={index}
                      className="text-[14px] font-semibold px-2 py-[1px] border border-gray-400 rounded-2xl hover:text-[#FF6300]"
                    >
                      {genre}
                    </Link>
                  ))}
        ...
        <div>
          {seasons?.length > 0 && (
            <div className="flex flex-col gap-y-7 mt-8">
              <h1 className="w-fit text-2xl text-[#FF6300] max-[478px]:text-[18px] font-bold">
                More Seasons
              </h1>
                  ...
                  <Link
                    ...
                    className={`relative w-[20%] h-[60px] rounded-lg overflow-hidden cursor-pointer group ${
                      currentId === String(season.id)
                        ? "border border-[#FF6300]"
                        : ""
                    } max-[1200px]:w-[140px] max-[575px]:w-full`}
                  >
                    <p
                      className={`text-[13px] text-center font-bold absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full px-2 z-30 line-clamp-2 group-hover:text-[#FF6300] ${
                        currentId === String(season.id)
                          ? "text-[#FF6300]"
                          : "text-white"
                      }`}
